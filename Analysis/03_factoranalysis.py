...
def FactorAnalysisFromDataFrame(df, n_factors):
    ...
    # 因子分析
    fa = FA(n_factors, rotation='promax', method='minres')
    fa.fit(z)
...
# 因子負荷量の絶対値がthresh未満の変数を消す
df_selected_factor_score, df_selected_factor_loading = df_factor_score, df_factor_loading
df_selected = df
selectedwords = [i[0] for i in  df_selected_factor_loading[np.logical_or(df_selected_factor_loading >= thresh,
                                                                         df_selected_factor_loading <= -1*thresh)].dropna(how='all').index]
...
df_selected_factor_score.to_csv(os.path.join(path_to_file, 'FA_factor_score_from_tfidf.csv'), 
                                             encoding='utf-8-sig') # 因子得点をCSV保存
df_selected_factor_loading.to_csv(os.path.join(path_to_file, 'FA_factor_loading_from_tfidf.csv'), 
